...
- **Phase 1 MVP: 実装完了・Windows 実機テスト済み** (テスト134件全通過)
- Phase 2: 未着手
...
- **勝手にコミットしない**（pre-commit フックがユーザー確認を強制する）
- コミットメッセージは英語で、変更の「why」を簡潔に記述する
- 大きな変更は小さなコミットに分割する

### コミットワークフロー

コミット前に以下のステップを順番に実行する。いずれかが失敗したら停止する。

#### 1. Lint
```bash
.venv/Scripts/python.exe -m ruff check src/ tests/
```
失敗したら修正して再実行。パスするまで次に進まない。
...
#### 3. Doc-sync（ドキュメント自動同期）

##### 3a. テスト件数同期
- ステップ2の pytest 出力からテスト通過件数を取得（例: "114 passed"）
- `CLAUDE.md` と `docs/handoff.md` のテスト件数表記（例: "テスト114件"）を確認
- 件数が異なれば Edit ツールで更新

##### 3b. ファイル構造同期
- `src/` 配下の `.py` ファイル一覧を Glob で取得
- `CLAUDE.md` の `## ファイル構造` セクションのツリーと比較
- ファイルの追加・削除があればツリーを更新（既存のコメント注釈は保持、新規ファイルには用途を簡潔に記述）

##### 3c. ドキュメント変更のステージング
- 3a/3b でドキュメントを更新した場合、`git add` でステージングに含める

#### 4. 教訓記録（MEMORY.md 更新）
- 今回の作業でデバッグや調査を通じて得た技術的教訓・知見があるか振り返る
- 教訓がある場合、`memory/MEMORY.md` に追記する（既存エントリと重複しないよう確認）
- 記録対象の例: API の落とし穴、ライブラリの挙動、デバッグ手法、環境固有の注意点
- 単純な機能追加やバグ修正の手順そのものは記録不要（再現性のある「知見」のみ）
